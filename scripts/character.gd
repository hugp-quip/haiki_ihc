...
func _physics_process(delta):
	...
	if get_viewport().get_camera_3d() == $fpcamera:
		rotate_camera(delta)
	...
	#Rotate camera to position of this.
	$Camera_Controller.rotation = $Camera_Controller.rotation.move_toward(rotation, 0.05)
	
	
	$Camera_Controller.position = $Camera_Controller.position.lerp(position, 0.2)
